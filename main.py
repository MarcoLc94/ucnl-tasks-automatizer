    ...
    try:
        ...
        for item in raw_items:
            ...
            if activity["type"] == "assignment":
                ...
                new_count += 1
                logger.info(f"Nueva tarea registrada: {task_title}")
                ...
                st.add_task(
                    course_name=course_name,
                    task_title=task_title,
                    task_description=f"Examen con {len(answered)} preguntas",
                    task_type="exam",
                    task_url=task_url,
                    exam_questions=exam_questions,
                )
                new_count += 1
                logger.info(f"Nuevo examen registrado: {task_title}")
...
async def _submit_task(task_id: str):
    ...
    try:
        if task.task_type == "assignment":
            ok = await submit_assignment(task.task_url, task.ai_response or "")
        else:
            questions = [
                {
                    "question": q.question,
...
async function loadTasks() {
  ...
  if (!tasks.length) {
    container.innerHTML = '<p class="text-gray-400 text-center py-10">No hay tareas registradas</p>';
    return;
  }
...
function renderTask(t) {
  ...
  if (t.task_type === 'assignment') {
    body = \`
      <div class="mt-3">
        <p class="text-xs font-semibold text-gray-500 mb-1">DESCRIPCIÓN DE LA TAREA</p>
    ...
    body = \`
      <div class="mt-3">
        <p class="text-xs font-semibold text-gray-500 mb-2">PREGUNTAS DEL EXAMEN (respuestas marcadas con ✓)</p>
        <div class="bg-gray-50 rounded p-3 max-h-64 overflow-y-auto">\${questions}</div>
      </div>
    \`;
  }

  const actions = t.status === 'pending_approval' ? \`
  ...
  return \`
    <div class="bg-white rounded-xl border shadow-sm p-5">
      <div class="flex items-start justify-between gap-4">
        <div>
          <p class="text-xs text-gray-400">\${t.course_name} · \${t.created_at}</p>
          <h2 class="text-base font-semibold text-gray-900 mt-0.5">\${t.task_title}</h2>
